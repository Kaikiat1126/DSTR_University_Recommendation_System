...
public:
    ...
    void insert(string key, int rank, string institution, string locationCode, string location, 
        double arScore, int arRank, double erScore, int erRank, double fsrScore, int fsrRank, 
        double cpfScore, int cpfRank, double ifrScore, int ifrRank, double isrScore, int isrRank, 
        double irnScore, int irnRank, double gerScore, int gerRank, double scoreScaled);
    void remove(string institution);
    UniversityRBTreeNode<T>* search(string institution, int method = 0);
    void print();
    void preOrder();
    void inOrder();
    void postOrder();
...
void UniversityRBTree::insert(string key, int rank, string institution, string locationCode, string location, double arScore, int arRank, double erScore, int erRank,
    double fsrScore, int fsrRank, double cpfScore, int cpfRank, double ifrScore, int ifrRank, double isrScore, int isrRank, double irnScore,
    int irnRank, double gerScore, int gerRank, double scoreScaled) {
    auto* element = new UniversityData<T>(rank, institution, locationCode, location, arScore, arRank, erScore, erRank,
        fsrScore, fsrRank, cpfScore, cpfRank, ifrScore, ifrRank, isrScore, isrRank, irnScore, irnRank, gerScore, gerRank, scoreScaled);
    auto* newNode = new UniversityRBTreeNode<T>(element, Red, nullptr, nullptr, nullptr);
...
void UniversityRBTree::remove(UniversityRBTreeNode*& root, UniversityRBTreeNode* node) {
    ...
    if (node->leftChild && node->rightChild) {
        UniversityRBTreeNode<T>* replace = node;
...
void UniversityRBTree::preOrder() {
    if (!root)
        ...
        timer.getEndTime();
        cout << "Time taken by preorder traversal: ";
...
void UniversityRBTree::inOrder() {
    if (!root)
        ...
        timer.getEndTime();
        cout << "Time taken by inorder traversal: ";
        cout << timer.getRunTime() << endl;
    }
}
...
void UniversityRBTree::postOrder() {
    if (!root)
        ...
        cout << "Time taken by postorder traversal: ";
        cout << timer.getRunTime() << endl;
    }
}
